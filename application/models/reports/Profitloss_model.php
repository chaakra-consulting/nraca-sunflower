...
class Profitloss_model extends CI_Model {
	...
	function get_data_akun_dapat() {
		$query = $this->db->query("SELECT * FROM `acc_coa_type` WHERE `deleted` = 0 AND account_type 
		-- in('Pendapatan','Beban Penjualan') 
		in('Pendapatan') 
		AND parent IS NULL ORDER BY account_number ASC");
		if($query->num_rows() > 0) {
			$out = $query->result();
			return $out;
		} else {
