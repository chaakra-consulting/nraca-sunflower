...
class Purchase_Payments_model extends Crud_model {
    ...
    function get_details($options = array()){
        ...
        if ($start_date) {
            $start_date = get_array_value($options, "start_date");
            $end_date = get_array_value($options, "end_date");  
            $where .= " AND (pay_date >='".$start_date."'AND  pay_date <='".$end_date."')";
        }
    ...
    function get_detailss($options = array()){
        ...
        $data = $this->db->query("SELECT * FROM $this->table WHERE  $invoices_table.deleted= '0' AND $invoices_table.paid= 'PAID' ".$where." ORDER BY id DESC");
        return $data;
    }
    ...
    /*function getInvoicesCust($id){
     ...
     function getInvoicesCust($id){
        $data = $this->db->query("SELECT
                            ...
                            WHERE
                                a.fid_invoices = b.id 
                                AND b.fid_vendor = $id AND b.paid in('Not Paid','CREDIT') AND b.status = 'posting' AND a.deleted = 0 AND b.deleted = 0 GROUP BY b.id DESC");
        return $data;
    }
