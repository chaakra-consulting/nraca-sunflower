...
class Messages extends MY_Controller {
    ...
    function modal_form($user_id = 0) {
        ...
        if ($this->login_user->user_type === "marketing") {
            ...
            if (in_array($this->login_user->id, $client_message_users_array)) {
                ...
                $users = $this->Users_model->get_team_members_and_clients("staff", "marketing
                ", $this->login_user->id)->result();
            ...
            if ($client_message_users) {
                ...
                $users = $this->Users_model->get_team_members_and_clients("staff","admin_penjualan", $client_message_users)->result();
        ...
        if ($user_id) {
            ...
            foreach ($users as $user) {
                ...
                if ($user->user_type === "admin_penjualan" && $user->company_name) {
            ...
            $view_data['users_dropdown'] = array("" => "-") + $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("user_type" => "admin_penjualan", "id !=" => $this->login_user->id));
