...
if(!function_exists('purchase_invoices')){

    function purchase_invoices(){
        $CI = get_instance();

        $invoices = $CI->db->query("SELECT purchase_invoices.code,purchase_invoices.amount,purchase_invoices.end_date,master_customers.name FROM purchase_invoices  JOIN master_customers ON master_customers.id = purchase_invoices.fid_vendor  WHERE purchase_invoices.paid = 'Not Paid' AND purchase_invoices.status = 'posting' and purchase_invoices.deleted = 0 LIMIT 5");

        $view_data['invoices'] = $invoices->result();
        $CI->load->view("widget/invoices/pendingg", $view_data);
    }
}

if(!function_exists('purchase_invoices')){

    function calculateTotalNominalPerMonth(){
        $CI = get_instance();

        $invoices = $CI->db->query('SELECT YEAR(inv_date) as year, MONTH(inv_date) as month, SUM(total) FROM purchase_invoices JOIN purchase_invoices_items ON purchase_invoices.id=purchase_invoices_items.fid_invoices WHERE  purchase_invoices.deleted=0 AND purchase_invoices.status="terverifikasi" GROUP BY YEAR(inv_date), MONTH(inv_date)')->row_object();
        $view_data['invoices'] = $invoices->result();
        $CI->load->view("widget/invoices/total", $view_data);
    }
}
